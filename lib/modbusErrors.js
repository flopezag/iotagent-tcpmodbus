...
function generate_fnc_code_field(mb_fnc_code) {
    ...
    if (mb_fnc_code < byte1 || mb_fnc_code > byte2) {
        ...
        let result = Buffer.from(code.toString(), 'hex');
...
function mb_exception_response(code, exception_code) {
    ...
    console.log(code.toString('hex'), "->", failed_fnc_code.toString('hex'));
    
    var mb_exception_rsp = Buffer.concat([failed_fnc_code, exception_code]);
    console.log(mb_exception_rsp.toString('hex'));

    return mb_exception_rsp;
}

// console.log(mb_exception_codes['01'].name, ": ", mb_exception_codes['01'].description);
// console.log('');
// console.log(mb_exception_codes['02'].name, ": ", mb_exception_codes['02'].description);

const byte1 = Buffer.from('05', 'hex');
const byte2 = Buffer.from('01', 'hex');

let response = mb_exception_response(byte1, byte2)
