...
function generate_fnc_code_field(mb_fnc_code) {
    /******************************************
     * Generate the Function Code Field, based on a bit operation 1000 000 or mb_fnc_code
     */

    if (mb_fnc_code > '7F' | mb_fnc_code < '01') {
        throw new Error('The function code should be between ["01", "7F"]')
    } else {
        const bitmask = '80'; // Bitmask in hexadecimal

        return bitmask | mb_fnc_code    
    }
}

function mb_exception_response(code, exception_code) {
    failed_fnc_code = generate_fnc_code_field(code);
    
    var buffer = Buffer.alloc(2);
    buffer.write([failed_fnc_code, exception_code]);
}
